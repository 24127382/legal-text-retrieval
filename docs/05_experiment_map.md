...
- **Data/corpus diagnostics:** schema/type/nullability violations; duplicate/missing IDs; null/empty values; split sizes/manifests/fingerprints; exact/near-duplicate questions; duplicated answers/references; task-aware passage/document overlap; gold-document distribution/maximum-5 ceiling; label-completeness signals; LegalQA gold-evidence availability.
...
- **Hypothesis:** identical inputs/config regenerate the same corpus mapping, và index/evaluation dùng đúng manifest.
- **Control:** recorded manifests/fingerprints; **independent variable:** regeneration/reload environment, không đổi research method.
- **Fixed components:** input bytes, preprocessing config/code revision, corpus/index serialization contract.
- **Data/corpus diagnostics:** unique IDs; no orphan/empty/unexplained duplicate; source-slice equality; document/chunk counts; deterministic IDs; no silent loss/skip; input/config/corpus/index fingerprints; `index count ↔ manifest count ↔ chunk IDs`.
- **Downstream LegalIR diagnostics:** identical-query smoke test hoặc ranking checksum chỉ để phát hiện index mismatch, không phải benchmark claim.
- **Expected signal:** zero unexplained mismatch và reproducible mapping.
- **Failure interpretation:** corpus/index drift, nondeterministic ID/order, stale artifact hoặc provenance corruption; block benchmark.
- **Dependency/status:** chạy khi freeze C0 và lặp cho mọi corpus/index variant; `Planned`.

`C0` được freeze từ một policy đơn giản sau D00–D02 và D06. D03–D05 là controlled representation ablations sau khi có fixed B0; variant thắng vẫn phải qua D06 trước khi dùng cho LegalIR comparison tiếp theo.

## Ma trận LegalIR E00–E16

`Official R/P` dưới đây là `recall`/`precision` đúng theo organizer scorer, không phải giả định priority. Candidate R@K, MRR và nDCG là diagnostics. Trừ experiment Data & Corpus được khai báo riêng, mọi LegalIR row giữ fixed C0 manifest/corpus fingerprint đã qua D06.
...
|---|---|---|---|---|---|---|---|
| E00 | BM25 tạo lexical floor hữu ích | No retriever → query tokenization/BM25 parameters | C0 corpus/split/fingerprint, document mapping, aggregation/final rule | Official R/P; candidate R@10/20/50/100/200; F01/F02 | Exact citation/terminology coverage | Low coverage: inspect representation/index unit through a separate D experiment | P00 + C0/D06; `Planned` |
| E01 | BGE-M3 dense bổ sung paraphrase coverage | E00 → retriever family | C0 corpus/split/fingerprint, mapping, aggregation/final rule | Official R/P; candidate R@K; unique gold vs BM25 | F03/F04 unique recovery | No gain/complement: encoding/domain gap or redundant errors | E00 + C0/D06; dense prototype `Code present`, experiment `Planned` |
...
|---|---|---|---|---|---|---|---|---|---|
| QA0 | Controls tách giá trị của evidence khỏi generator | Closed-book; gold-evidence control nếu mapping reliable | Evidence condition: none vs gold | Same generator, prompt, decoding, output adapter; gold-evidence adapter fixed khi chạy oracle | `rouge`, `meteor` | Evidence coverage if mapping exists; support rate; answer length | Reliable gold evidence improves grounded answer over closed-book | No gap: generator/prompt ceiling, weak mapping, or task not evidence-limited; không suy ra retrieval tốt | P00 + QA data; oracle conditional on verified gold mapping; `Planned` |
| QA1 | Retrieve–rerank–generate improves over no-rerank RAG | Same retriever/generator without reranking | Reranking and evidence order/subset | Canonical evidence records, generator, prompt, decoding, context budget, submission adapter | `rouge`, `meteor` | Candidate/evidence coverage; selected support; retrieval/selection/generation error split | Better selected evidence and official scores | No gain: coverage ceiling, reranker mismatch, generator insensitive or metric variance | LegalIR B2 benchmark on C0/D06; `Planned` |
| QA2 | Extract-then-generate reduces distractor noise | QA1 full-source-chunk context | Evidence selector/extracted source spans | Retrieved canonical records, source-preserving evidence, generator, prompt, decoding, budget | `rouge`, `meteor` | Span coverage/precision if labels; claim support; omitted-condition errors | Higher support/conciseness without evidence loss | Selector drops conditions or adds no value over chunks | QA1; `Planned` |
...
- **Commit / config / input data and split fingerprint / seed:**
